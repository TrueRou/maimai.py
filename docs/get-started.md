...
- 在更新成绩时，使用水鱼用户名和密码：`PlayerIdentitifer(username="Username", credentials="Password")`。
- 在更新成绩时，使用水鱼Import-Token：`PlayerIdentitifer(credentials="Import-Token")`。
- 在更新 / 获取成绩时，使用落雪好友代码：`PlayerIdentitifer(friend_code="")`
