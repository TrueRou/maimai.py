...
在更新成绩时，使用个人Import-Token：`PlayerIdentitifer(credentials="Import-Token")`，将使用个人API。

在更新成绩时，使用水鱼用户名和密码：`PlayerIdentitifer(username="Username", credentials="Password")`，将使用个人API。
